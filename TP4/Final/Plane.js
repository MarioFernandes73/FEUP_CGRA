...
function Plane(scene, nrDivs, textureRatio=1, boardRatio=1) {
	...
	this.k = boardRatio/textureRatio;
	this.initBuffers();
};
	...
	*/
	...
	for (var j = 0; j <= this.nrDivs; j++) 
	{
		...
		for (var i = 0; i <= this.nrDivs; i++) 
		{
			...
			if(this.k>1) {
				increment = this.k/this.nrDivs;
				limit = (this.k-1)/2;
				this.texCoords.push(-limit+i*increment, j*this.patchLength);
			}
			else if (this.k<1){
				increment = (1/this.k)/this.nrDivs;
				limit = ((1/this.k)-1)/2;
				this.texCoords.push(i*this.patchLength, -limit+j*increment);
			}
			else
